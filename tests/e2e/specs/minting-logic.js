...
describe('Tests related to minting logic', () => {
    ...
    it('Quantity limit value is correctly updated once the user mint the NFT', function () {
        ...
        mintingPage.mintingModal
            .infoMessage('NFT successfully claimed. You can continue minting.')
            .should('be.visible');
        mintingPage.mintingModal
            .nftBalancepPerPhase()
            .invoke('text')
    ...
    it('NFT total price is correct according to quantity value', function () {
        ...
        mintingPage.mintingModal
            .nftPrice()
            .should('contain', `Total price: ${Cypress.env('nftPrice')} MATIC`);
        for (let i = 2; i <= Cypress.env('nftClaimLimit'); i++) {
            mintingPage.clickOnPlusButton();
            mintingPage.mintingModal
                .nftPrice()
                .should('contain', `Total price: ${i * Cypress.env('nftPrice')} MATIC`);
        }
    });
    ...
    it("The 'Number of claimed NFTs' info updates once the user claims the NFT", function () {
        ...
        mintingPage.mintingModal
            .nftBalancepPerPhase()
            .should('contain', `1 of ${Cypress.env('nftClaimLimit')} NFT claimed`);
    });

    it('User can mint the minimum allowed number of NFTs to mint', function () {
        // Switch account
        ...
        cy.confirmMetamaskTransaction();
        mintingPage.mintingModal
            .infoMessage('Waiting for the receipt')
            .should('be.visible');
        mintingPage.mintingModal
            .infoMessage('Succesfully claimed. Preparing, please wait')
            .should('be.visible');
        mintingPage.mintingModal
            .infoMessage('NFT successfully claimed. You can continue minting.')
            .should('be.visible');
        cy.wait(2000);
        // Get id of the minted NFT displayed in the Minting Modal
        mintingPage.mintingModal
            .claimedNftBtn()
            .invoke('attr', 'data-nft-id')
            .then((nftID) => {
                // Get name of the minted NFT displayed in the Minting Modal
                mintingPage.mintingModal
                    .claimedNftBtn()
                    ...
                    .then((alt) => {
                        ...
                        mintingPage.nftCardModal
                            .nftCardTitle()
                            .should('have.text', nftName);
                        mintingPage.nftCardModal
                            .nftImage()
                            .invoke('attr', 'alt')
                            .then((modalImgAlt) => {
                                // Verify that correct NFT image is displayed in the minted NFT modal
    ...
    it('User can mint one by one the maximum allowed number of NFTs to mint', function () {
        ...
        for (let i = 0; i < Cypress.env('nftClaimLimit'); i++) {
            ...
            if (i < Cypress.env('nftClaimLimit') - 1) {
                ...
                mintingPage.mintingModal.infoMessage(
                    'NFTs successfully claimed. Maximum NFTs per wallet minted.'
                );
            }
            cy.wait(2000);
    ...
    it('User can mint the maximum allowed number of NFTs to mint', function () {
        ...
        mintingPage.mintingModal
            .nftBalancepPerPhase()
            .contains(
                `${Cypress.env('nftClaimLimit')} of ${Cypress.env('nftClaimLimit')} NFTs claimed`
            );
        cy.wait(2000);
        mintingPage.mintingModal
            .claimedNftBtn()
            ...
            .then((nftID) => {
                mintingPage.mintingModal
                    ...
                    .then((alt) => {
                        ...
                        cy.log(nftName);
                        mintingPage.clickOnclaimedNftButton();
                        cy.get('.font-bold').contains(
                            `1 of ${Cypress.env('nftClaimLimit')} freshly minted NFTs shown.`
                        );
                        mintingPage.nftCardModal.nftId().should('have.text', nftID);
                        mintingPage.nftCardModal
                            .nftCardTitle()
    ...
    it.only("The 'Number of claimed NFTs by user' info updates once the user claims the NFT", function () {
        cy.switchMetamaskAccount('account 5').then((switched) => {
            expect(switched).to.be.true;
        });
        mintingPage.clickOnConnectWalletButton();
        ...
        mintingPage.mintingModal
            ...
            .then((clameBalanceInfoSentence) => {
                ...
                mintingPage.mintingModal
                    ...
                    .should('be.visible');
                cy.wait(2000);
                const parts = clameBalanceInfoSentence.split('of');
                const claimedNFTsInfo = parts[0].trim();
                const numberOfClaimedNFTs = Number(claimedNFTsInfo);
                mintingPage.mintingModal
                    .nftBalancepPerPhase()
                    .should(
                        'contain',
                        `${numberOfClaimedNFTs + 1} of ${Cypress.env('nftClaimLimit')} NFT claimed`
                    );
            });
    });
    ...
    it("The 'Total claimed NFTs' info update correctly once the user mint the NFT", function () {
        ...
        cy.getTotalClaimedValue().then((totalClaimedValue) => {
            ...
            mintingPage.mintingModal
                ...
                .should('be.visible');
            cy.get('p').contains(
                `Claiming an NFT is like putting a digital trophy on your virtual shelf. Total claimed NFTs: ${
                    totalClaimed + 1
                }`
            );
        });
            ...
            mintingPage.header
                ...
                .then((numberOfNfts) => {
                    ...
                    mintingPage.header
                        .numberOfNftsBtnFigure()
                        .invoke('text')
                        .then((numberOfNftsAfterMinting) => {
                            const numberOfNFTsBeforeMinting = Number(numberOfNfts);
                            const numberOfNFTsAfterMinting = Number(
                                numberOfNftsAfterMinting
                            );
                            // Verify that 'Total claimed NFTs by user' info from 'Profile' button us updated
                            expect(numberOfNFTsAfterMinting).to.equal(
                                numberOfNFTsBeforeMinting + 1
                            );
                        });
                });
        });
