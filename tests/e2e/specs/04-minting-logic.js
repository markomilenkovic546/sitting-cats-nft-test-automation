...
describe('Tests related to minting logic', () => {
    ...
    it('User cannot set the quantity greater than number of NFTs allowed to mint', function () {
        mintingPage.clickOnConnectWalletButton();
        cy.acceptMetamaskAccess();
        mintingPage.mintingModal.nftBalancepPerPhase.invoke('text').then((text) => {
    ...
    it("The 'Number of claimed NFTs' info updates once the user claims the NFT", function () {
        ...
        mintingPage.mintingModal
            .infoMessage('NFT successfully claimed. You can continue minting.')
            .should('be.visible');
        mintingPage.mintingModal.nftBalancepPerPhase.should(
            'contain',
            `1 of ${Cypress.env('nftClaimLimit')} NFT claimed`
        );
    });
    ...
    it('User can mint the minimum allowed number of NFTs to mint', function () {
        ...
        mintingPage.mintingModal.claimedNftBtn.invoke('attr', 'data-nft-id').then((nftID) => {
            // Get name of the minted NFT displayed in the Minting Modal
            mintingPage.mintingModal.claimedNftBtn
                ...
                .then((alt) => {
                    ...
                    mintingPage.nftCardModal.nftImage.invoke('attr', 'alt').then((modalImgAlt) => {
    ...
    it('User can mint the maximum allowed number of NFTs to mint', function () {
        ...
        mintingPage.mintingModal
            ...
            .should('be.visible');
        mintingPage.mintingModal.nftBalancepPerPhase.contains(
    ...
    it.only("The 'Number of claimed NFTs by user' info updates once the user claims the NFT", function () {
        ...
        cy.acceptMetamaskAccess();
        mintingPage.mintingModal.infoMessage('Ready for minting').should('be.visible');
        mintingPage.mintingModal.nftBalancepPerPhase.invoke('text').then((clameBalanceInfoSentence) => {
            mintingPage.clickOnMintButton();
            mintingPage.mintingModal
                .infoMessage('Please confirm transaction in your wallet to continue.')
                .should('be.visible');
    ...
    it("The 'Total claimed NFTs by user' info from 'Profile' button updates correctly once the user mint the NFT", function () {
        ...
        cy.getTotalClaimedValue().then((totalClaimedValue) => {
            ...
            mintingPage.header.numberOfNftsBtnFigure.invoke('text').then((numberOfNfts) => {
